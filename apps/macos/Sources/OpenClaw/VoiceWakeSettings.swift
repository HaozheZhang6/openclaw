    ...
    private var unsupportedVoiceWakePanel: some View {
        HStack(alignment: .top, spacing: 12) {
            ...
            VStack(alignment: .leading, spacing: 4) {
                ...
                Text(
                    "Voice Wake uses Apple's macOS 26 Speech framework APIs. " +
                        "This Mac is running macOS \(Self.currentMacOSVersion); update to macOS 26 or newer to enable these controls.")
                    .font(.footnote)
                    .foregroundStyle(.secondary)
                    .fixedSize(horizontal: false, vertical: true)
            }
